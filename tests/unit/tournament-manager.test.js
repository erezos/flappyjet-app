...
describe('TournamentManager', () => {
  ...
  describe('Tournament Creation', () => {
    test('should create weekly tournament with default settings', async () => {
      ...
      // Mock the create_weekly_tournament function call
      mockDb.query
  ...
  describe('Score Submission', () => {
    ...
    test('should submit tournament score and update leaderboard', async () => {
      ...
      mockDb.query
        ...
        .mockResolvedValueOnce({
          rows: [{ rank: 1 }] // Player rank query
        })
        .mockResolvedValueOnce({
          rows: [{ id: 'event-logged' }] // Event logging
        })
        .mockResolvedValueOnce({
          rows: [
    ...
    test('should not update if score is not better', async () => {
      mockDb.query
        ...
        .mockResolvedValueOnce({
          rows: [{ id: 'updated-games-count' }] // Just increment games played
        });
      ...
      expect(mockDb.query).toHaveBeenCalledTimes(2); // Participant check + games increment
    });
  });
  ...
  describe('Tournament Lifecycle', () => {
    ...
    test('should start tournament and notify participants', async () => {
      mockDb.query
        .mockResolvedValueOnce({
          rows: [{ id: mockTournamentId, name: 'Test Tournament' }] // Tournament update
        })
        .mockResolvedValueOnce({
          rows: [
    ...
    test('should end tournament and distribute prizes', async () => {
      ...
      // Mock additional queries for tournament end process
      mockDb.query
        .mockResolvedValueOnce({
          rows: [{ id: mockTournamentId }] // Tournament status update
        });

      // Mock leaderboard snapshot creation (3 times for 3 players)
      for (let i = 0; i < 3; i++) {
        mockDb.query.mockResolvedValueOnce({
          rows: [{ rank: i + 1 }] // Player rank query
        });
        mockDb.query.mockResolvedValueOnce({
          rows: [{ id: 'snapshot-created' }] // Snapshot creation
        });
      }

      // Mock event logging
      mockDb.query.mockResolvedValueOnce({
        rows: [{ id: 'event-logged' }]
      });
