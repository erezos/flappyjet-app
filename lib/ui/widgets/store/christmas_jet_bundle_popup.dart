/// 🎄 Christmas Jet Bundle Special Offer Popup
/// 
/// Beautiful, engaging popup for the Christmas tournament special offer
/// Shows 3 exclusive jet skins (Blitzen, Comet, Rudolph) for $1.99
...
/// Show the Christmas jet bundle special offer popup
/// 
...
Future<bool?> showChristmasJetBundlePopup({
  ...
  return showDialog<bool>(
    ...
    builder: (dialogContext) => PopScope(
      ...
      child: ChristmasJetBundlePopupWidget(
        onPurchaseComplete: () {
...
/// Christmas jet bundle popup widget
class ChristmasJetBundlePopupWidget extends StatefulWidget {
  final VoidCallback onPurchaseComplete;
  final VoidCallback onDismiss;

  const ChristmasJetBundlePopupWidget({
    super.key,
  ...
  @override
  State<ChristmasJetBundlePopupWidget> createState() => _ChristmasJetBundlePopupWidgetState();
}

class _ChristmasJetBundlePopupWidgetState extends State<ChristmasJetBundlePopupWidget>
  ...
  final String _productId = 'christmas_jet_bundle';

  // Jet skins in the bundle
  static const List<String> _jetSkinIds = ['blitzen', 'comet', 'rudolph'];
  static const String _equipSkinId = 'blitzen';
  ...
  Future<void> _handlePurchase() async {
    ...
    try {
      ...
      if (iapProduct == null) {
        safePrint('⚠️ Christmas jet bundle product not found');
        if (mounted) {
      ...
      safePrint('⚠️ Error purchasing Christmas bundle: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<void> _handlePurchaseSuccess() async {
    ...
    try {
      safePrint('🎉 Christmas Bundle purchase completed - unlocking skins...');
      ...
      EventBus().fire('special_offer_purchased', {
        'offer_id': 'christmas_jet_bundle',
        'product_id': 'christmas_jet_bundle',
        'price_usd': 1.99,
      ...
      if (mounted) {
        ...
        Future.delayed(const Duration(milliseconds: 300), () {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: const Text('🎉 Christmas bundle purchased! All 3 jets unlocked! Blitzen equipped!'),
