...
class GameConfig {
  ...
  static const double maxFallSpeed = 500.0;  // ULTRA GENTLE: Much slower max fall speed
  static const double jetRotationSpeed = 2.0;  // ULTRA GENTLE: Very smooth visual rotation
  
  // === START SCREEN SETTINGS ===
  static const double startScreenJetXRatio = 0.20;   // 🎯 PERFECT FIX: 20% from left edge (ensures full jet visibility with center anchor)
  static const double startScreenJetYRatio = 0.30;   // 🎯 SIMPLE FIX: 30% from top (middle screen height)
  ...
  static double getStartScreenJetX(double screenWidth) => screenWidth * startScreenJetXRatio;
  
  /// 🔥 SIMPLE FIX: Position jet at screen center (50%) - always visible and properly positioned
  static double getStartScreenJetY(double screenHeight) => screenHeight * startScreenJetYRatio;
  ...
  static const double obstacleSpeed = 45.0;   // ULTRA SLOW: Beginner-friendly speed
  static const double obstacleGap = 320.0;    // MASSIVE: Even more generous gap 
  static const double obstacleSpawnInterval = 4.0;   // SLOWER: More time between obstacles
  
  // === SCORING SYSTEM ===
  static const int pointsPerObstacle = 1;     // Points awarded per obstacle
  static const int bonusPointsPerTheme = 5;   // Bonus points for theme unlock
  
  // === LIFE SYSTEM ===
  static const int maxLives = 3;              // Maximum lives
  static const double invulnerabilityDuration = 3.5;  // PRODUCTION: Very generous recovery time
  ...
  static const double difficultyIncreaseRate = 0.002; // ULTRA GENTLE: Barely noticeable progression for first 50 points
  static const double maxDifficultyMultiplier = 2.0;  // REDUCED: More reasonable maximum difficulty
  static const double minObstacleGap = 220.0;  // MAJOR INCREASE: Even at max difficulty, stays very playable
  static const double gapReductionRate = 0.08;  // ULTRA SLOW: Gap barely shrinks, maintains confidence
  
  // === THEME SYSTEM ===
  static const List<GameTheme> themes = GameThemes.allThemes;
  static const double themeTransitionDuration = 3.0;  // Theme notification duration
  static const double themeUnlockBonusMultiplier = 2.0;  // Score multiplier on unlock
  ...
  static const bool debugCollisionRects = false; // Disable collision debug drawing for production
  ...
  static const double continueOfferDuration = 10.0;  // Seconds to show continue offer
