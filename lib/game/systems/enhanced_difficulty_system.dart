...
class EnhancedDifficultySystem {
  ...
  static void debugPrintDifficulty(int score) {
    final phase = getPhaseForScore(score);
    debugPrint('🎯 DIFFICULTY: Score $score → ${phase.toString()}');
    debugPrint('🎨 VISUAL: Background=${getBackgroundTheme(score)}, Ground=${getGroundTheme(score)}, Obstacles=${getObstacleTheme(score)}');
    debugPrint('🎵 AUDIO: Theme=${getAudioTheme(score)}');
  }
}