...
class TapEffect {
  ...
  void _renderEngineGlow(Canvas canvas) {
    ...
    canvas.drawCircle(Offset(position.x, position.y), glowSize * 2, outerGlow);
    ...
    canvas.drawCircle(Offset(position.x, position.y), glowSize, innerGlow);

    // Core
    final core = Paint()..color = color.withValues(alpha: glowAlpha);

    canvas.drawCircle(Offset(position.x, position.y), glowSize * 0.5, core);
  }
  ...
  void _renderEnergyRing(Canvas canvas) {
    ...
    canvas.drawCircle(Offset(position.x, position.y), ringRadius, ringPaint);
    ...
    canvas.drawCircle(Offset(position.x, position.y), ringRadius, glowPaint);
  }
      ...
      if (waveProgress > 0) {
        final wavePaint = Paint()
          ..color = color.withValues(
            alpha: (1.0 - waveProgress) * intensity * 0.5,
          )
          ..style = PaintingStyle.stroke
          ..strokeWidth = 3;
  ...
  void _renderRealJetEngine(Canvas canvas) {
    ...
    flamePath.moveTo(
      position.x - fireLength * 0.1,
      position.y - fireWidth * 0.4,
    );
    flamePath.lineTo(
      position.x - fireLength * 0.7,
      position.y - fireWidth * 0.2,
    );
    ...
    outerFlamePath.moveTo(
      position.x - fireLength * 0.2,
      position.y - fireWidth * 0.5,
    );
    outerFlamePath.lineTo(
      position.x - fireLength * 0.9,
      position.y - fireWidth * 0.3,
    );
    ...
    if (progress < 0.5) {
      ...
      for (int i = 0; i < particleCount; i++) {
        ...
        final particleX =
            position.x -
            particleDistance +
            math.cos(particleAngle) * fireWidth * 0.2;
        final particleY =
            position.y + math.sin(particleAngle) * fireWidth * 0.3;

        final particlePaint = Paint()
          ..color = Colors.yellow.withValues(
            alpha: engineAlpha * (1.0 - progress) * 0.7,
          )
