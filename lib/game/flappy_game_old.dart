...
// Removed _StubParticleSystem - using HardwareParticleSystem instead
...
class FlappyGame extends FlameGame {
  ...
  void render(Canvas canvas) {
    ...
    // No fallback needed - HardwareParticleSystem is always initialized
  ...
  // DirectParticle fallback system removed - using HardwareParticleSystem only
  late List<dynamic> _directParticles;
  ...
  Future<void> _initializeMCPSystems() async {
    ...
    // Initialize hardware particle system asynchronously
    _hardwareParticleSystem.preRenderParticles().then((_) {
      safePrint('🚀 HardwareParticleSystem initialized - Ready for crash effects!');
    }).catchError((e) {
      safePrint('⚠️ HardwareParticleSystem initialization failed: $e');
    });
  ...
  void update(double dt) {
    ...
    // 🚀 HARDWARE-ACCELERATED: HardwareParticleSystem handles its own updates
    // No fallback needed - HardwareParticleSystem is always initialized
  ...
  bool get hasDirectParticles => false; // HardwareParticleSystem handles all particles
  int get directParticleCount => 0; // No direct particles anymore
  List<dynamic> getDirectParticles() => []; // No direct particles anymore
  void clearDirectParticles() {} // No direct particles to clear
  ...
  void testCreateDirectExplosion(Vector2 position, int count) {
    // HardwareParticleSystem handles all particle effects
    _hardwareParticleSystem.createCrashBurst(position);
  }
  ...
  /// Celebration burst: hardware-accelerated stars, confetti, and colorful particles
  ...
  /// Crash burst: hardware-accelerated smoke puffs + sparks/embers
  ...
  /// Sparkle confetti: hardware-accelerated confetti and stars
  void _createSparkleConfetti(Vector2 center) {
    // 🚀 HARDWARE-ACCELERATED: Use pre-rendered sprites for maximum performance
    // Create a special celebration burst with extra sparkles for milestones
    final extraSparkles = _score % 10 == 0 ? 12 : 0;
    _hardwareParticleSystem.createCelebrationBurst(center, _score + extraSparkles);
