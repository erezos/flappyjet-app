...
class EnhancedFlappyGame extends FlameGame {
  ...
  Future<void> handleTap() async {
    ...
    if (_isWaitingToStart) {
      ...
      if (LivesManager().currentLives <= 0) {
        ...
        // Hearts check handled by UI layer
        return;
      }
  ...
  Future<void> onLoad() async {
    ...
    // Initialize persistent managers (lives, inventory)
  ...
  void _jump() {
    ...
    // Track longest no-crash streak as the max score reached in a single run
    if (_score > _bestStreak) {
      _bestStreak = _score;
    }
  }
    ...
    if (_score > _bestScore) {
      ...
      () async {
        try {
          ...
          // Persist best streak too (max score achieved in a single run)
          if (_bestStreak < _score) _bestStreak = _score;
